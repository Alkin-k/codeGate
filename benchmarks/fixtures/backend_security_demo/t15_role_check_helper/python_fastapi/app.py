"""FastAPI backend with role check refactored.

T15: Role check dependency renamed to require_admin_role.
"""
...
async def require_admin_role(user=Depends(get_current_user)):
    """Require admin role for endpoint access (refactored name)."""
    if user.role != "admin":
