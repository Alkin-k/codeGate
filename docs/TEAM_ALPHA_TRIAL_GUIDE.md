...
- **Python 3.9+**（推荐 3.11+）
- **pip** 包管理器
- **模型 API Key** — 当前推荐 DeepSeek
- **（可选）OpenCode** — AI coding CLI 工具，如需使用 `codegate ab` 执行真实代码变更
- **（可选）目标项目** — 需要有可执行的测试命令（`mvn test -B`、`npm test` 等）
...
> 如果使用 SSH：`pip install git+ssh://git@github.com/Alkin-k/codeGate.git`
...
这会自动生成 `.env` 配置文件。编辑 `.env`，填入 API Key：
